...
function torpedoPickV2(state, ship, enemies, TC) {
  ...
  for (var i = 0; i < enemies.length; i++) {
    ...
    var sc = dist(ship.x, ship.y, e.x, e.y)
           ...
           + (focus && e.id === focus.id ? -120 : 0); // focus is a TIE-BREAK for torpedoes:
                                                  //   at -400 it bought 400px of extra flight,
                                                  //   and flight time (PD windows + evasion
                                                  //   time) costs more than concentration buys
                                                  //   on an interceptable weapon
